...
function renderYearSummary(data) {
  ...
  // ✅ Render table including "Missing" column
  const headerRow = Object.keys(yearCounts).map(y => `<th>${y}</th>`).join("");
  const valueRow = Object.values(yearCounts).map(v => `<td>${v}</td>`).join("");
